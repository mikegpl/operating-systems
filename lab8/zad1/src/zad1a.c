...
void *threadJob(void *arg) {
    ...
    TRYSSERT("ferror", ferror(fileHandle), 0, "Coulnd't read from file");

    TRYSSERT("mutex_unlock", pthread_mutex_unlock(&mutex), 0, "Couldn't unlock mutex");

    while (recordsRead > 0) {
        for (int i = 0; i < recordsRead; i++) {
            if (&recordArray != NULL && strstr(recordArray[i].text, word) != NULL) {
    ...
    printf("Thread id [%ld]: phrase not found\n", myId);
    return (void *) 0;
}
